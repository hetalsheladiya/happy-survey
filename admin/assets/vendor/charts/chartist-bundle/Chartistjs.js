...
function getBarChart(storeId){     
    
    var dataToSend = {"userId": userId, 'storeId': storeId }; 
    
    $.ajax({
        url: server+"/surveySummary.php",
        type: 'GET',
        data: dataToSend,
        ...
        success: function(response){
            var result = response.data;
                countt = count;
            var categoryNames = Object.keys(result);

                if(categoryNames.length == 0){
                    $('.ct-chart-multilines.ct-golden-section').html('There are no categories. Click on "Categories" Tab to create custom categories.');
                }
                else{  
                    categoryNames = Object.keys(result);
                    categoryRatings = [];
                    /*categorySadCount = [];
                    ...
                    allSurveyCount(categoryName, categorySadCount, categoryNeautralCount, categoryHappyCount);*/
                    categoryNames.forEach(function(catName) {
                        categoryRatings.push(result[catName]);
                    })
                    drawBarChart(categoryNames, categoryRatings);
                    $('.ct-chart-multilines.ct-golden-section').html("");                
                }
...
function drawBarChart(categoryNames, categoryRatings){    
...
function allSurveyCount(categoryName, categorySadCount, categoryNeautralCount, categoryHappyCount){    
    // if ($('.ct-chart-multilines').length) {
        new Chartist.Bar('.ct-chart-multilines', {
            labels: categoryName,
            series: [
